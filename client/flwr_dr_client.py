...
parser.add_argument("--environment_name", default="Eplus-5zone-mixed-discrete-stochastic-v1", help="The name of the environment in the OpenAI Gym framework")
...
def create_env(env_name, experiment_name, yearS=1991, monthS=1, dayS=1, yearE=1992, monthE=1, dayE=1):
    ...
    #env = CSVLogger(env)
    #env = ReduceObservationWrapper(env, ['month','day_of_month','hour','outdoor_temperature','outdoor_humidity','wind_speed','wind_direction','diffuse_solar_radiation','direct_solar_radiation','clg_setpoint','air_humidity','people_occupant','co2_emission', 'HVAC_electricity_demand_rate', 'total_electricity_HVAC']) #only htg_setpoint and air_temperature
    #env = ReduceObservationWrapper(env, ['day_of_month', 'wind_direction', 'diffuse_solar_radiation', 'co2_emission', 'HVAC_electricity_demand_rate', 'total_electricity_HVAC'])
...
class SinergymClient(fl.client.Client):
    def __init__(self, seed, environment_name, episodes, episode_len, grammar_angle, input_space, types, constant_range, constant_step, with_bias, lambda_, generations, jobs, cxp, mp, mutation, crossover, genotype_len, selection, patience, s_day, s_month, s_year, e_day, e_month, e_year, random_init, n_actions, df, eps, decay, low, up, rng, timeout, env_seed):
        self.seed = seed
        ...
        self.experiment_name = f"SB3_PPO-{environment_name}_{datetime.today().strftime('%Y-%m-%d_%H:%M')}"
    ...
    def get_parameters(self, ins):
        ...
        type_bytes = pickle.dumps([self.best_genotype,self.best_phenotype])
        parameters = Parameters(tensors=[type_bytes], tensor_type="DEAP_Individual")
    ...
    def set_parameters(self, parameters):
        type_bytes = parameters.tensors[0]
        received_types = pickle.loads(type_bytes)
        self.server_genotype = received_types[0]
        self.server_phenotype = received_types[1] # change to test nonfl
    ...
    def fit(self, ins):
        ...
        previous = []
        if self.best_genotype is not None: #test nonfl
            previous.append(self.best_genotype)
            print('==== Previous best genotype used for training ====')
        if self.server_genotype is not None:
            previous.append(self.server_genotype)
            print('==== Server genotype used for training ====')
        ...
        #print(f"best_fitness: {hof[0].fitness.values[0]}")
        
        if self.best_genotype is None or hof[0].fitness.values[0] > self.best_genotype.fitness.values[0]:
            self.best_genotype = hof[0]
        ...
        with open(resfile, "a") as f:
            f.write(f"run best:{hof[0].fitness.values[0]}; genotype:{hof[0]}\n top best:{self.best_genotype.fitness.values[0]}; genotype:{self.best_genotype}\n")
        
        
        return FitRes(
            status=Status(code=Code.OK, message="Training complete"),
            parameters=self.get_parameters(GetParametersIns(ins.config)).parameters,
            num_examples=num_evaluation_episodes,
            metrics={"fitness": self.best_genotype.fitness.values[0]},
        )

    def evaluate(self, ins):
        # Update the model with the received parameters for evaluation
        self.set_parameters(ins.parameters)
        
        if self.server_phenotype == '':
            ...
            phenotype = self.server_phenotype
            print('==== Server phenotype used for evaluation ====')
        ...
        print(f"==== Eval fitness: {-fitness_score[0]} ====")
        num_evaluation_episodes = self.episodes
        
        #calc best local phenotype eval score anyway, if after first round, TODO maybe put in for 0 to 1 above
        if self.best_phenotype != '':
            random.seed(self.seed)
            np.random.seed(self.seed)
            global_cumulative_rewards = []
            ...
            try:
                for iteration in range(self.episodes):
                    ...
                    for t in range(self.episode_len):
                        ...
                        action = program(self.best_phenotype, input_)
            ...
            fitness_score2 = fitness
            print(f"==== Local Eval fitness: {-fitness_score2[0]} ====")
            num_evaluation_episodes = self.episodes
        
        return EvaluateRes(
            status=Status(code=Code.OK, message="Eval complete"),
            loss=-fitness_score[0],
            num_examples=num_evaluation_episodes,
            metrics={"fitness": fitness_score[0]},
        )
        
    def train_GE(self, leaf, genotype, gen=0, progr=False):
        if self.angle == 1:
        ...
        try:
            for iteration in range(self.episodes):
                ...
                for t in range(self.episode_len):
                    ...
                    if (self.server_phenotype == '') or (progr == False):
                        action = bt(obs)
                    else:
                        input_ = []
                        input_.extend(obs)
                        printed_d=True
                        action = program(self.server_phenotype, input_)

                    previous = obs[:]

                    #with HiddenPrints(): #didn't work well with some parallelization as far as i remember, sinergym output has been cut at docker init
                    obs, rew, done, trunc, info = e.step(action)
...
if __name__ == "__main__":
    ...
    start = 1
    factor = 1000
    seeds_offset = [(start + i) * factor + args.seed for i in range(args.clients)] #has some problems, use sh to start more copies of the script instead
    
    with parallel_backend("multiprocessing"):
        clients = [
            SinergymClient(
                environment_name=args.environment_name.split('§'),
                seed=seed_unique,
            ...
            for seed_unique in seeds_offset
        ]
        
        # Start Flower client
        Parallel(args.clients, timeout=args.timeout)(
            delayed(fl.client.start_client)(
                #server_address='flower-server:8080', #using docker bridge
